...
export const CONCEPTS_INSTRUCTION = `Задача: пользователь описал идею для фотосъёмки. Придумай 3-4 РАЗНЫЕ визуальные концепции, которые отличаются по подходу (например: тёмный минимализм / фактурная естественность / графичный высокий контраст) — не вариации одной и той же идеи. Для каждой концепции заполни: title (например "Concept 01 — Dark Minimalism"), mood, location, background, lighting, props (с функцией каждого), composition, colorPalette (3-5 цветов в hex с названиями), moodDescription (развёрнутое описание того, как это выглядит визуально), searchPrompts (3-5 готовых поисковых запросов на английском для Pinterest/Behance/Google Images), referenceNotes (на что смотреть в референсах).`;
